...
function getMessageCellHtml(row) {
  return escapeHtml(safeString(row?.message));
}
...
export function setEmptyTrackingTable(trackingTableBody) {
  ...
  trackingTableBody.innerHTML = `
    <tr class="tracking-empty-row">
      <td colspan="5">업로드된 파일이 없습니다. 엑셀 파일을 선택해 주세요.</td>
    </tr>
  `;
}
...
export function syncRowsFromTable(trackingTableBody) {
  ...
  return tableRows.map((row, index) => {
    ...
    const messageCell = row.querySelector('td[data-field="message"]');
    ...
    return {
      ...
      message: safeString(messageCell?.textContent),
